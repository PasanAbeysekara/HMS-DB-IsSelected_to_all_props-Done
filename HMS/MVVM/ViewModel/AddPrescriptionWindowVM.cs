	...
	public partial class AddPrescriptionWindowVM : ObservableObject, ICloseWindows
	{
		...
		void ExecuteCreateDrugCommand()
		{
			...
			using (DataContext context = new DataContext())
			{
				...
				MessageBox.Show($"Drug Id:{drg.Id}\nDrug Name:{DrugName}\nDose:{Dose}\npresc ID:{prescripId}\ncommennts:{Comments}");
		...
		void ExecuteDoneCommand()
		{
			...
			MessageBox.Show("Please click 'Refresh' to see the updated Prescription list 😊");
			Close?.Invoke();
		...
		public AddPrescriptionWindowVM()
		{
			...
			using (DataContext context = new DataContext())
			{
				...
				Caption = $"Prescription for patient '{context.Patients.Single(x => x.IsPatientSelected == true).FullName}'";
			}
