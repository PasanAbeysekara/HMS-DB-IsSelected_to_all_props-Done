	...
	public partial class AddAppointmentWindowVM : ObservableObject, ICloseWindows
	{
		...
		void ExecuteSaveCommand()
		{
			using (DataContext context = new DataContext())
			{
				...
				//var _DoctorId = context.Doctors.Single(x => x.Name == doctorName).Id;
				//var _PatientId = context.Patients.Single(x => x.IsPatientSelected == true).Id;
				//var _AppointedDate = AppointmentDate;
				//MessageBox.Show($"Doc Id : {_DoctorId}\nPatient Id :{_PatientId}\n_Appointment Id : {_AppointedDate}");
			}
			MessageBox.Show("Please click 'Refresh' to see the updated Appointment list 😊");
			Close?.Invoke();
